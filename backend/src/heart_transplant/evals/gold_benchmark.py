...
from heart_transplant.evals.gold_audit import accepted_blocks_for_row, active_gold_rows, audit_gold_rows
...
def run_benchmark(structural: dict[str, Any], gold_items: list[dict[str, Any]]) -> dict[str, Any]:
    """Compare heuristic labels to accepted gold blocks for listed ``node_id``s."""
    art = StructuralArtifact.model_validate(structural)
    active_items = active_gold_rows(gold_items)
    scoped_items = [item for item in active_items if gold_item_applies_to_artifact(item, art)]
    ...
    for g in scoped_items:
        accepted = accepted_blocks_for_row(g)
        expected = str(g.get("primary_block") or accepted[0])
        candidates = nodes_for_gold_item(g, art, by_id)
        if not candidates:
            rows.append({**g, "expected_block": expected, "accepted_blocks": accepted, "got": None, "match": False, "error": "missing node"})
            continue
        ...
        for node in candidates:
            ...
            ok = ok or bool(set(accepted).intersection({str(block) for block in got_blocks}))
        if ok:
            correct += 1
        rows.append(
            {
    ...
    return {
        ...
        "active_input_total": len(active_items),
        "skipped_non_active": len(gold_items) - len(active_items),
        "skipped_repo_scope": len(active_items) - len(scoped_items),
        "correct": correct,
...
def build_block_benchmark_report(
    ...
    *,
    ...
    gold_health = audit_gold_rows(gold_items, gold_set_path=gold_set_path)
    rows = raw["rows"]
    ...
    duplicate_gold_keys = [
        ...
        for key, count in Counter(
            (
                ...
                str(item.get("primary_block") or item.get("expected_block", "")),
            )
    ...
    return {
        ...
        "summary": {
            ...
            "active_gold_rows": raw["active_input_total"],
            "skipped_non_active": raw["skipped_non_active"],
            ...
            "gold_health_status": gold_health["summary"]["overall_status"],
            "gold_needs_review_rows": gold_health["summary"]["needs_review_rows"],
            "gold_multi_label_rows": gold_health["summary"]["multi_label_row_count"],
            "gold_missing_required_field_count": gold_health["summary"]["missing_required_field_count"],
            "gold_active_contradictory_target_count": gold_health["summary"]["active_contradictory_target_count"],
        },
