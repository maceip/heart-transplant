    ...
    *,
    ...
    for code_node in structural.get("code_nodes", []):
        ...
        nodes[node_id] = {
            ...
            "provenance": provenance(
                str(code_node.get("symbol_source")) if code_node.get("symbol_source") == "scip" else "tree_sitter",
                source_artifact=manifest.source_artifacts.get("scip" if code_node.get("symbol_source") == "scip" else "structural"),
                source_id=node_id,
            ),
            "meta": {
                "symbol_source": code_node.get("symbol_source"),
    ...
    for assignment in semantic.get("block_assignments", []) or []:
        ...
        nodes[semantic_id] = {
            ...
            "provenance": provenance("semantic_classifier", source_artifact=manifest.source_artifacts.get("semantic"), source_id=source),
            "meta": {
                "confidence": assignment.get("confidence"),
                "secondary_blocks": assignment.get("secondary_blocks", []),
        ...
        edges.append(
            canonical_edge(
                ...
                provenance=provenance("semantic_classifier", source_artifact=manifest.source_artifacts.get("semantic"), source_id=semantic_id),
            )
        )
        for secondary in assignment.get("secondary_blocks", []) or []:
            ...
            edges.append(
                canonical_edge(
                    ...
                    "SECONDARY_DESCRIBES",
                    structural.get("repo_name"),
                    provenance=provenance("semantic_classifier", source_artifact=manifest.source_artifacts.get("semantic"), source_id=secondary_id),
                )
            )
        ...
        if source and target:
            edges.append(
                canonical_edge(
                    ...
                    "IMPLEMENTS",
                    structural.get("repo_name"),
                    provenance=provenance("scip", source_artifact=manifest.source_artifacts.get("scip"), source_id=source),
                    meta=item,
                )
            )
